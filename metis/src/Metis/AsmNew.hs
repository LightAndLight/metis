...
import Metis.IsaNew (Instruction, Isa, Register, Symbol (..))
...
data Statement isa
  = Instruction (Instruction isa (Register isa))
  | Directive Directive

deriving instance (Isa isa) => Eq (Statement isa)
deriving instance (Isa isa) => Show (Statement isa)
...
printAsm ::
  (Instruction isa (Register isa) -> Builder) ->
  Asm isa ->
  Builder
